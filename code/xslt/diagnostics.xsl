...
<!--  Various collections inside the root. -->
  <xsl:variable name="rawHtml" select="collection(concat($docRoot, '/?*.hocr;recurse=yes'))//html"/>
  <xsl:variable name="htmlFiles" select="collection(concat($docRoot, '/?*.html;recurse=yes'))"/>
  <xsl:variable name="editedHtml" select="$htmlFiles//html[contains(document-uri(.), 'hocr_edited')]"/>
  <xsl:variable name="teiDebateFiles" select="collection(concat($docRoot, '/?*.xml;recurse=yes'))//tei:TEI[@type='full']"/>
          ...
          <table>
            <tbody>
              <tr>
                ...
                <td><xsl:value-of select="count($rawHtml)"/></td>
              </tr>
              <tr>
                <td>Edited HOCR pages:</td>
                <td><xsl:value-of select="count($editedHtml)"/></td>
              </tr>
              <tr>
                <td>Pages in TEI:</td>
